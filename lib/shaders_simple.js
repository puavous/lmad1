 ...
 *           i[3].xyz   (unused)          i[3].w  mesh brightness
 * 
 * vec4 b - "Background color"
 * 
 * "Fog" is a mix between computed color and background color.
 * Expect value between 0-1 as the "amount". Effect becomes max at "far"
 * and the slope is a power..
 */
var frag_shader_vanilla14 =
        ...
        'void main(){' +
    	    ...
    	    'gl_FragColor=vec4(' +
                ...
                '+max(i[0].rgb,' +
                     // Diffuse reflection
                     'i[1].rgb*max(0.,dot(normalize(l),normalize(n)))' +
                        // Specular reflection
                        '+i[2].rgb*pow(max(0.,dot(normalize(r),' +
                                                 'reflect(-normalize(l),normalize(n))' +
                                                 ')' +
                                          '),' +
                                       'i[2].a' +
